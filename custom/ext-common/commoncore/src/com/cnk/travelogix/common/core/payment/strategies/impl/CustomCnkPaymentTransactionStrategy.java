...
public class CustomCnkPaymentTransactionStrategy implements CnkPaymentTransactionStrategy
{
	...
	@Override
	public PaymentTransactionEntryModel savePaymentTransactionEntry(final CustomerModel customerModel, final CartModel cart,
			final PaymentInfoModel paymentInfoModel, final PaymentTransactionData txnData)
	{
		validateParameterNotNull(txnData, "txnData cannot be null");

		final PaymentTransactionModel transaction = getModelService().create(PaymentTransactionModel.class);
		final PaymentTransactionType paymentTransactionType = txnData.getType();
		transaction.setCode(customerModel.getUid() + "_" + UUID.randomUUID());
		transaction.setPlannedAmount(BigDecimal.valueOf(txnData.getAmount()));
		transaction.setInfo(paymentInfoModel);
		transaction.setOrder(cart);
		transaction.setPaymentProvider(txnData.getPaymentProvider());
		transaction.setPaymentMode(txnData.getModeOfPayment());
		getModelService().save(transaction);

		final PaymentTransactionEntryModel entry = getModelService().create(PaymentTransactionEntryModel.class);
		entry.setType(paymentTransactionType);
		entry.setTime(new Date());
		entry.setAmount(BigDecimal.valueOf(txnData.getAmount()));
		entry.setPaymentTransaction(transaction);
		entry.setTransactionStatus(TransactionStatus.ACCEPTED.name());
		entry.setTransactionStatusDetails(TransactionStatusDetails.SUCCESFULL.name());
		entry.setCode(getPaymentService().getNewPaymentTransactionEntryCode(transaction, paymentTransactionType));
	...
	public void setPaymentTransactionReviewResult(final PaymentTransactionEntryModel reviewDecisionEntry, final String guid)
	{
		...
		final PaymentTransactionModel transaction = order.getPaymentTransactions().iterator().hasNext()
				? order.getPaymentTransactions().iterator().next() : null;

		if (checkIfAuthorizationWaitForReview(transaction))
		{
			reviewDecisionEntry.setPaymentTransaction(transaction);
			reviewDecisionEntry.setCode(
					getPaymentService().getNewPaymentTransactionEntryCode(transaction, PaymentTransactionType.REVIEW_DECISION));
