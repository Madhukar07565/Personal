 ...
 *
 */
public class TravelogixIVRPaymentInfoReversePopulator implements Populator<CCPaymentInfoData, PaymentInfoModel>
{
	...
	public void populate(final CCPaymentInfoData source, final PaymentInfoModel target) throws ConversionException
	{
		...
		/*if (source.getModeOfPayment().equalsIgnoreCase("cashondelivery"))
		{
			final CashOnDeliveryPaymentInfoModel cashOnDeliveryPaymentInfoModel = new CashOnDeliveryPaymentInfoModel();
			...
			//cashOnDeliveryPaymentInfoModel.setDenomination(source.getde);
		}
		if (source.getModeOfPayment().equalsIgnoreCase("creditcard"))
		{
			...
			//cardPaymentInfoModel.setCcOwner(source);
			cardPaymentInfoModel.setValidToMonth(source.getExpiryMonth());
