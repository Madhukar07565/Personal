...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String ORDER_LIST_CMS_PAGE = "cnkOrderListPage";
	...
	@Resource(name="travelogixCmsComponentFacade")
	private TravelogixCmsComponentFacade travelogixCmsComponentFacade;
	...
	/**
	 * This is meant for displaying Order List page with Dynamic orders data
	 * 
	 * @return String
	 * @throws CMSItemNotFoundException 
	 */
	@RequestMapping(value = "/orderList", method = RequestMethod.GET)
	public String getOrderListPage(final Model model) throws CMSItemNotFoundException
	{
		
		LOG.info("---------getOrderListPage()------------called :");
		
		List<OrderData> orderList=orderListFacade.getListOfOrders();
		model.addAttribute("orderList",orderList);
		
		storeCmsPageInModel(model, getContentPageForLabelOrId(ORDER_LIST_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ORDER_LIST_CMS_PAGE));
	...
	public String profile(final Model model) throws CMSItemNotFoundException
	{
		...
		//call facade to get all the CMS components and flag indicating user belongs to user group or not at Home page 
		final Map<AbstractCMSComponentModel,String> compWithUserGroupMemberFlag = travelogixCmsComponentFacade.getAllComponentsOnHomepage();
		model.addAttribute("compWithUserGroupMemberFlag", compWithUserGroupMemberFlag);
	
		model.addAttribute("customerData", customerData);
		
		//need to get employee from current login user 
		final EmployeeData employeeData = employeeFacade.getCurrentEmployee();
		model.addAttribute("employeeDatavalue", employeeData);
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		else
		{
			...
			{
				LOG.error(passwordMismatchException.getMessage(),passwordMismatchException);
	...
	@ResponseBody
	@RequestMapping(value = "/saveCompForUserPref", method = RequestMethod.POST)
	public String saveSelectedCompToUserPreference(@RequestParam("compId") final String compId, 
			@RequestParam("check") final boolean check)
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("from saveSelectedCompToUser ajax call method " + compId);
		}
		return travelogixCmsComponentFacade.saveComponentToUserGroup(compId, check);
	}
}