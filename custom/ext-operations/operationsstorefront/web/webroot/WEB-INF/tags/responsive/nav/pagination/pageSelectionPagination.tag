	...
	<ul class="pagination pagination-sm">
		<c:if test="${hasPreviousPage}">
			<li><spring:url value="${searchUrl}" var="previousPageUrl"
					...
					<a class="doToPaginationTag" href="${previousPageUrl}">&laquo;</a>
				</ycommerce:testId></li>
		</c:if>
		...
		<c:forEach begin="${beginPage}" end="${endPage}" var="pageNumber">
			...
			<c:choose>
				<c:when
					...
					<c:choose>
						<c:when
							test="${ (searchPageData.pagination.currentPage + 1 eq beginPage) or (searchPageData.pagination.currentPage + 1 eq endPage) }">
							<c:if
								test="${searchPageData.pagination.currentPage + 1 eq beginPage}">
								<c:if
									test="${pageNumber gt searchPageData.pagination.currentPage + 3}">
									<c:set var="linkClass" value="hidden-xs" />
								</c:if>
							</c:if>

							<c:if
								test="${searchPageData.pagination.currentPage + 1 eq endPage}">
								<c:if
									test="${pageNumber lt searchPageData.pagination.currentPage - 1}">
									<c:set var="linkClass" value="hidden-xs" />
								</c:if>
							</c:if>
						</c:when>
						<c:otherwise>
							<c:if
								test="${pageNumber lt searchPageData.pagination.currentPage}">
								<c:set var="linkClass" value="hidden-xs" />
							</c:if>

							<c:if
								test="${pageNumber gt searchPageData.pagination.currentPage + 2}">
								<c:set var="linkClass" value="hidden-xs" />
							</c:if>
						</c:otherwise>
					</c:choose>
					...
					<ycommerce:testId code="pageNumber_link">
						<li><a class="doToPaginationTag" href="${pageNumberUrl}">${pageNumber}</a></li>
					</ycommerce:testId>
				</c:when>
				<c:otherwise>
					<li class="active">
		...
		<c:if test="${hasNextPage}">
			<li class="pagination-next"><spring:url value="${searchUrl}"
					var="nextPageUrl" htmlEscape="true">
					<spring:param name="page"
						value="${searchPageData.pagination.currentPage + 1}" />
				</spring:url> <ycommerce:testId code="searchResults_nextPage_link">
					<a class="doToPaginationTag" href="${nextPageUrl}">&raquo;</a>
				</ycommerce:testId></li>
		</c:if>
	</ul>
</c:if>