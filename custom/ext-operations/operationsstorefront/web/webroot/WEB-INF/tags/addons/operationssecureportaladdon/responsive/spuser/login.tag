...
<div class="main">
	<div class="container">
		...
		<section id="signin" class="signin">
			<div class="col-sm-4 col-xs-12 signin-block">
				<form:form action="${action}" method="post" commandName="loginForm" class="form-signin">
					<c:if test="${not empty message}">
						<span class="has-error"> <spring:theme code="${message}" />
						</span>
					</c:if>
		...
		<ycommerce:testId code="login_forgotPassword_link"> --%>
					...
					<a href="<c:url value='/login/pw/request'/>" class="float-right"
					...
					<%-- <div class="row login-form-action">
						<c:set var="loginBtnClasses"
							value="col-sm-12 col-md-6 col-md-push-6" /> --%>
