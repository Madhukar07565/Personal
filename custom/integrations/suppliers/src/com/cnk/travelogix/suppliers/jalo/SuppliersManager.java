...
/**
 * This is the extension manager of the Suppliers extension.
 */
public class SuppliersManager extends GeneratedSuppliersManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties
     * 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(SuppliersManager.class.getName());
	...
	return (SuppliersManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
		.getExtension(SuppliersConstants.EXTENSIONNAME);
    }
     ...
     */
    public SuppliersManager() // NOPMD 
    {
	if (LOG.isDebugEnabled()) {
	    LOG.debug("constructor of SuppliersManager called.");
	}
    }
    ...
    public void init() {
	if (LOG.isDebugEnabled()) {
	    LOG.debug("init() of SuppliersManager called. " + getTenant().getTenantID());
	}
    }
    ...
    public void destroy() {
	if (LOG.isDebugEnabled()) {
	    LOG.debug("destroy() of SuppliersManager called, current tenant: " + getTenant().getTenantID());
	}
    }
