...
import com.cnk.travelogix.cart.impl.CommerceWebServicesCartFacade;
import com.cnk.travelogix.exceptions.InvalidPaymentInfoException;
import com.cnk.travelogix.exceptions.NoCheckoutCartException;
import com.cnk.travelogix.exceptions.UnsupportedDeliveryModeException;
import com.cnk.travelogix.exceptions.UnsupportedRequestException;
import com.cnk.travelogix.order.data.CartDataList;
import com.cnk.travelogix.order.data.OrderEntryDataList;
import com.cnk.travelogix.product.data.PromotionResultDataList;
import com.cnk.travelogix.request.support.impl.PaymentProviderRequestSupportedStrategy;
import com.cnk.travelogix.stock.CommerceStockFacade;
import com.cnk.travelogix.voucher.data.VoucherDataList;
...
public class CartsController extends BaseCommerceController
{
			...
			@RequestParam(required = false) final String sort)
	{
		...
		{
			cartDataList.setCarts(getCartFacade().getCartsForCurrentUser());
		}
			...
			@RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		if (StringUtils.isNotEmpty(oldCartId))
		{
			...
			try
			{
				getCartFacade().restoreAnonymousCartAndMerge(oldCartId, toMergeCartGuid);
				return getDataMapper().map(getSessionCart(), CartWsDTO.class, fields);
			}
			...
			@RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		final DeliveryModeListWsDTO dto = getDataMapper().map(deliveryModesData, DeliveryModeListWsDTO.class, fields);
		return dto;
	}
