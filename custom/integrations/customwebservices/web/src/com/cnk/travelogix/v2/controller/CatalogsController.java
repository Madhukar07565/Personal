...
public class CatalogsController extends BaseController
{
	...
	public CatalogListWsDTO getCatalogs(@RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		final CatalogListWsDTO catalogListDto = getDataMapper().map(catalogsData, CatalogListWsDTO.class, fieldSet);
			...
			@RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		final CatalogWsDTO dto = getDataMapper().map(catalogData, CatalogWsDTO.class, fieldSet);
		return dto;
	}
			...
			@PathVariable final String catalogVersionId, @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		...
		final CatalogVersionWsDTO dto = getDataMapper().map(catalogVersionData, CatalogVersionWsDTO.class, fieldSet);
		return dto;
	}
			...
			@RequestParam(defaultValue = "DEFAULT,totalNumber,pageSize,numberOfPages,currentPage") final String fields)
	{
		...
		final CategoryHierarchyWsDTO dto = getDataMapper().map(categoryHierarchyData, CategoryHierarchyWsDTO.class, fieldSet);
		return dto;
	}
