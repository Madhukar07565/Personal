...
public class ProductPageController extends AbstractPageController
{
	...
	private static final String HOTEL_DETAIL_PAGE = "ezgHotelDetailsPage";
	...
	@Resource(name = "facetHotelSearchFacade")
	private CnkProductSearchFacade<HotelData> facetHotelSearchFacade;

	@RequestMapping(value = "/hotel" + PRODUCT_CODE_PATH_VARIABLE_PATTERN + "/query", method = RequestMethod.POST)
	public String getProductDetailsInfo(@PathVariable("productCode") final String productCode,
			@RequestParam(value = "tabName", defaultValue = "hotel") final String tabName, final Model model,
			final HttpServletRequest request) throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		//search by hotelId actually but we will use the search data to filter suitable rooms of the target hotel
		final String widgetAndFacetSearchJson = request.getParameter("widgetAndFacetSearchJson");
		HotelSearchData widgetAndFacetSearchData = CnkBeanUtil.getBeanFromJson(widgetAndFacetSearchJson, HotelSearchData.class);

		//pdp page modify search widget
		//get mdf search data model
		String mdfSearchDataModel = request.getParameter("mdfSearchDataModel");
		if (StringUtils.isEmpty(mdfSearchDataModel))
		{
			mdfSearchDataModel = this.buildInitMdfSearchDataJson(widgetAndFacetSearchData);
		}
		HotelSearchData mdfSearchData = CnkBeanUtil.getBeanFromJson(mdfSearchDataModel, HotelSearchData.class);

		//if we search again by mdf search,should update search conditions
		updateWidgetAndFacetSearchData(mdfSearchData, widgetAndFacetSearchData);
		widgetAndFacetSearchData.setProductId(productCode);

		final HotelData hoteData = facetHotelSearchFacade.searchProductDetail(widgetAndFacetSearchData);
		model.addAttribute("hotelData", hoteData);
		model.addAttribute("mdfSearchDataModel", mdfSearchDataModel);
		model.addAttribute("hotelSearchData", widgetAndFacetSearchData);
		model.addAttribute("widgetAndFacetSearchJson", widgetAndFacetSearchJson);
		model.addAttribute("tabName", tabName);
		//used only when from cart page to PDP.
		model.addAttribute("originalGroupId", request.getParameter("originalGroupId"));
		storeCmsPageInModel(model, cmsPageService.getPageForId(HOTEL_DETAIL_PAGE));
		return getViewForPage(model);
	}

	private void updateWidgetAndFacetSearchData(HotelSearchData source, HotelSearchData target)
	{
		if (null != source && null != target)
		{
			target.setCheckInDate(source.getCheckInDate());
			target.setCheckOutDate(source.getCheckOutDate());
			target.setHotelSearchRooms(source.getHotelSearchRooms());
			target.setNoOfRooms(Integer.valueOf(source.getHotelSearchRooms().size()));
			target.setNights(DateUtil.getIntervalDays(source.getCheckInDate(), source.getCheckOutDate()));
		}
	}

	private String buildInitMdfSearchDataJson(HotelSearchData widgetAndFacetSearchData)
	{
		HotelSearchData mdfSearchData = new HotelSearchData();
		mdfSearchData.setCheckInDate(widgetAndFacetSearchData.getCheckInDate());
		mdfSearchData.setCheckOutDate(widgetAndFacetSearchData.getCheckOutDate());
		mdfSearchData.setNoOfRooms(widgetAndFacetSearchData.getNoOfRooms());
		mdfSearchData.setHotelSearchRooms(widgetAndFacetSearchData.getHotelSearchRooms());
		mdfSearchData.setProductId(widgetAndFacetSearchData.getProductId());
		mdfSearchData.setNights(widgetAndFacetSearchData.getNights());

		String mdfSearchDataModel = CnkBeanUtil.getJsonFromObject(mdfSearchData);
		return mdfSearchDataModel;
	}
