...
import com.cnk.travelogix.b2c.storefront.forms.CardInfoForm;
...
import com.cnk.travelogix.b2c.storefront.validation.EzgProfileUpdateValidator;
...
public class AccountPageController extends AbstractSearchPageController implements GenericFormUtil
{
	...
	private static final String SETTINGS_CMS_PAGE = REDIRECT_PREFIX + "/my-account/settings";
	...
	private static final String DASHBOARD_CMS_PAGE = "ezgb2cDashboardPage";
	private static final String DASHBOARD_EDIT_CMS_PAGE = "ezgb2cDashboardEditPage";

	private static final Logger LOG = Logger.getLogger(AccountPageController.class);

	@Resource(name = "b2cOrderFacade")
	private CNKOrderFacade orderFacade;
	...
	@Resource(name = "ezgProfileUpdateValidator")
	private EzgProfileUpdateValidator ezgProfileUpdateValidator;

	@Resource(name = "creditCardFacade")
	private B2cCreditCardFacade creditCardFacade;
	...
	public String getWishlist(final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		model.addAttribute("active", customerData.isActive());
	...
	public String getWallet(final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		...
		final List<CreditCardData> creditCard = creditCardFacade.getCreditCards();
		final List cardsForm = MultiCopyValue(creditCard, CardInfoForm.class);
		final GenericForm<CreditCardData> gf = new GenericForm<>();
		gf.setData(cardsForm);
		gf.setMeta(new CardInfoForm());
		model.addAttribute("monthList", initMonthList(request));
		model.addAttribute("cardList", writeToJson(gf));

		model.addAttribute("cardTypeList", writeToJson(buildValidateCardType()));
	...
	@RequestMapping(value = "/wallet/deleteCard/{cardNumber:.*}", method = RequestMethod.POST)
	@ResponseBody
	public ResponseMessage deleteCard(final HttpServletRequest request, @PathVariable("cardNumber") final String cardNumber)
			throws CMSItemNotFoundException
	{
		creditCardFacade.deleteCreditCard(cardNumber);
	...
	public ResponseMessage addCard(final HttpServletRequest request, final Model model,
			@RequestBody final CardInfoForm requestBody) throws CMSItemNotFoundException
	{
		final ResponseMessage responseMessage = new ResponseMessage();

		if (!requestBody.validate(responseMessage))
		{
		...
		try
		{
			...
			creditCardFacade.addCreditCard(copyValue(requestBody, CreditCardData.class));
		}
		catch (final ValidationFailedException e)
		{
			return new ResponseMessage(e);
		}
		return responseMessage.changeStatus("OK");
	}

	@RequestMapping(value = "/wallet/updateCard", method = RequestMethod.POST)
	@ResponseBody
	public ResponseMessage updateCard(final HttpServletRequest request, final Model model,
			@RequestBody final CardInfoForm requestBody) throws CMSItemNotFoundException
	{
		final ResponseMessage responseMessage = new ResponseMessage();
		if (!requestBody.validate(responseMessage))
		...
		creditCardFacade.updateCreditCard(copyValue(requestBody, CreditCardData.class));
		return responseMessage.changeStatus("OK");
	}

	/**
	 * this part should retrieve from back end configure,this is a workaround
	 */
	private List<CreditCardType> buildValidateCardType()
	{
		return Arrays.asList(CreditCardType.AMEX, CreditCardType.DINERS, CreditCardType.MAESTRO, CreditCardType.MASTER,
				CreditCardType.RUPAY, CreditCardType.VISA);
	}

	@RequestMapping(value = "/dashboard", method = RequestMethod.GET)
	public String getDashboard(final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		...
		final ProfileData profileData = customerFacade.getProfile();
		model.addAttribute("profileData", profileData);

		final CustomerData customerData = customerFacade.getCurrentCustomer();
		model.addAttribute("active", customerData.isActive());

		return "pages/ezg/account/dashboardPage";
	}

	@RequestMapping(value = "/dashboard_edit", method = RequestMethod.GET)
	public String getDashboardEdit(final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		storeCmsPageInModel(model, getContentPageForLabelOrId(DASHBOARD_EDIT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(DASHBOARD_EDIT_CMS_PAGE));
		...
		return "pages/ezg/account/dashboardPage_edit";
	}

	@RequestMapping(value = "/dashboard/updateProfile", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, String> updateProfile(final ProfileData profileData, final BindingResult bindingResult,
			final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		final Map<String, String> result = new HashMap<>();
		ezgProfileUpdateValidator.validate(profileData, bindingResult);
		final List<FieldError> allErrors = bindingResult.getFieldErrors();
		if (allErrors != null && allErrors.size() != 0)
		{
			for (final FieldError error : allErrors)
			{
				if ("areaCode".equals(error.getField()))
				{
					result.put("areaCode" + error.getCode(), error.getCode());
				}
				else if ("mobileNumber".equals(error.getField()))
				{
					result.put("mobileNumber" + error.getCode(), error.getCode());
				}
				else
				{
		...
		customerFacade.updateProfile(profileData);
	...
	public String getBookLists(@RequestParam(value = "page", defaultValue = "0") final int page,
			@RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
			@RequestParam(value = "sort", required = false) final String sortCode, final Model model) throws CMSItemNotFoundException
	{
		// Handle paged search results
		final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
		SearchPageData<OrderData> searchOrder = orderFacade.searchOrder(new OrderSearchCondition(), pageableData);
		populateModel(model, searchOrder, showMode);

		model.addAttribute("orderDataJson", escapeHtml(writeToJson(searchOrder)));
		try
		{
			model.addAttribute("productTypes", constructEnumOption(ProductTypeEnum.BUS));
			model.addAttribute("orderStatus", constructEnumOption(OrderStatus.APPROVED));
		}
		...
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return "pages/ezg/cart/bookingList";// getViewForPage(model);
	}
	
	private <T extends HybrisEnumValue> Map constructEnumOption(T targetEnum) throws IllegalArgumentException, IllegalAccessException{
		List<T> values = getValues(targetEnum);
		Map productTypeMap = new HashMap();
		for(HybrisEnumValue pte:values ){
			productTypeMap.put(pte.getCode(), pte.getCode().toLowerCase());
		}
		return productTypeMap;
	}
	
	
	private <T extends HybrisEnumValue> List<T> getValues(T ev) throws IllegalArgumentException, IllegalAccessException{
			
		List<T> list = new ArrayList<T>();
		Class<? extends HybrisEnumValue> class1 = ev.getClass();
		Field[] declaredFields = class1.getDeclaredFields();
		
		for (Field field : declaredFields)
		{
			if(field.getType().equals(class1)){
				list.add((T)field.get(ev));
			}
		}
		return list;
	}
	...
	@RequestMapping(value = "/bookList/search", method = RequestMethod.GET)
	@ResponseBody
	public SearchPageData<OrderData> searchBookLists(@RequestParam(value = "page", defaultValue = "0") final int page,  
			@RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
			@RequestParam(value = "sort", required = false) final String sortCode, 
			@RequestParam(value = "bookingID", required = false) final String bookingID, 
			@RequestParam(value = "passengerName", required = false) final String passengerName, 
			@RequestParam(value = "productName", required = false) final String productName, 
			@RequestParam(value = "productType", required = false) final String productType, 
			@RequestParam(value = "bookingStatus", required = false) final String bookingStatus, 
			final Model model) throws CMSItemNotFoundException
	{
		final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
		OrderSearchCondition queryParam = constructQueryParam(bookingID,passengerName, productName,productType,bookingStatus);
		SearchPageData<OrderData> searchOrder = orderFacade.searchOrder(queryParam, pageableData);
				
//		final List orderDetailsList = new ArrayList();
//		if (!searchOrder.getResults().isEmpty())
//		{
//			for (final Iterator iterator = searchOrder.getResults().iterator(); iterator.hasNext();)
//			{
//				final OrderHistoryData order = (OrderHistoryData) iterator.next();
//				final OrderData orderDetails = orderFacade.getGroupedOrderDetailsForCode(order.getCode());
...
//		}
	...
	private OrderSearchCondition constructQueryParam(String bookingID, String passengerName,String productName, String productType,String bookingStatus){
		
		OrderSearchCondition osc = new OrderSearchCondition();
		osc.setBookingID(bookingID);
		osc.setPassengerName(passengerName);
		osc.setProductName(productName);
		if(StringUtils.isNotEmpty(bookingStatus)){
			osc.setBookingStatus(OrderStatus.valueOf(bookingStatus));
		}
		
		if(StringUtils.isNotEmpty(productType)){
			osc.setProductType(ProductTypeEnum.valueOf(productType));
		}
		return osc;
	}
	...
	//	private void convertOrderData(final Model model, final OrderData order)
	//	{
	//
	//		final OrderEntryData entry = order.getEntries().get(0);
	//		final OrderHotelDetailInfoData hotel = (OrderHotelDetailInfoData) entry.getOrderProductInfo();
	//
	//		if (hotel != null)
	//		{
	//			final RoomData room = new RoomData();
	//			room.setAdultNo(hotel.getNumberOfAdult());
	//			room.setChildNo(hotel.getNumberOfChild());
	//			room.setCheckInDate(hotel.getHotelCheckinDate().toString());
	//			room.setCheckOutDate(hotel.getHotelCheckoutDate().toString());
	//			room.setNoOfRooms(hotel.getNumberOfrooms());
	//			final long diff = hotel.getHotelCheckoutDate().getTime() - hotel.getHotelCheckinDate().getTime();
	//
	//			model.addAttribute("nights", TimeUnit.DAYS.convert(diff, TimeUnit.MILLISECONDS));
	//			model.addAttribute("hotelData", room);
	//
	//			final List<OrderRoomDetailInfoData> rooms = hotel.getRoomDetailInfoList();
	//			if (!rooms.isEmpty())
	//			{
	//				final OrderRoomDetailInfoData rinfo = rooms.get(0);
	//
	//				final AccommodationTravellerData trveler = rinfo.getHotelTraveler();
	//
	//				final HotelTravelerData travelerForPage = new HotelTravelerData();
	//
	//				travelerForPage.setFirstName(trveler.getFirstName());
	//				travelerForPage.setLastName(trveler.getLastName());
	//
	//				final List<AccommodationNonPrimaryTravellerData> nonTrav = trveler.getNonPrimaryTravellerList();
	//				if (nonTrav != null)
	//				{
	//					if (!nonTrav.isEmpty() && nonTrav.get(0) != null)
	//					{
	//						travelerForPage.setSecondTravelerFirstName(nonTrav.get(0).getFirstName());
	//						travelerForPage.setSecondTravelerLastName(nonTrav.get(0).getLastName());
	//					}
	//					if (nonTrav.size() > 1 && nonTrav.get(1) != null)
	//					{
	//						travelerForPage.setChildFirstName(nonTrav.get(1).getFirstName());
	//						travelerForPage.setChildLastName(nonTrav.get(1).getLastName());
	//					}
	//
	//				}
	//				model.addAttribute("travelerData", travelerForPage);
	//			}
	//
	//		}
	//	}
