...
import com.cnk.travelogix.b2c.storefront.controllers.pages.checkout.steps.PaymentMethodCheckoutStepController;


@Controller
@RequestMapping(value = "/checkout/multi/payment/sop")
public class EzgSopPaymentResponseController extends PaymentMethodCheckoutStepController
{
	private static final Logger LOGGER = Logger.getLogger(EzgSopPaymentResponseController.class);
	...
	{
		final Map<String, String> fields = getRequestParameterMap(request);
		//		final AmexPaymentProvider amex = new AmexPaymentProvider();
		//		final PaymentServiceResponse resp = amex.interpretPaymentResponse(fields);
		...
		final PaymentSubscriptionResultData paymentSubscriptionResultData = this.getPaymentFacade()
				.completeSopCreateSubscription(fields, savePaymentInfo);
