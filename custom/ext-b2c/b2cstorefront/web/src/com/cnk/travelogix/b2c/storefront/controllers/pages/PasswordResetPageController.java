...
public class PasswordResetPageController extends AbstractPageController
{
	...
	private static final String RESETPASSWORD_PAGE = "ezgb2cResetPasswordPage";
	...
	@ResponseBody
	public ResponseMessage resetPassword(@RequestParam(required = false) final String userId, final HttpServletRequest request,
			final Model model) throws CMSItemNotFoundException
	{
		final ResponseMessage responseMessage = new ResponseMessage();
		if (StringUtils.isBlank(userId))
		{
			return responseMessage.changeStatus("Error");
		}
		else
		{
			if (customerFacade.checkUserExist(userId) == false)
			{
				return responseMessage.changeStatus("Error");
			}
		}

		customerFacade.processForgetPassword(userId);
		return responseMessage.changeStatus("OK");
	}

	@RequestMapping(value = "/enableAccountRequest", method = RequestMethod.POST)
	@ResponseBody
	public ResponseMessage enableAccount(@RequestParam(required = false) final String userId_enableAccount,
			final HttpServletRequest request, final Model model) throws CMSItemNotFoundException
	{
		final ResponseMessage responseMessage = new ResponseMessage();
		if (StringUtils.isBlank(userId_enableAccount))
		{
			return responseMessage.changeStatus("Error");
		}
		else
		{
			if (customerFacade.checkUserExist(userId_enableAccount) == false)
			{
				return responseMessage.changeStatus("Error");
			}
		}

		customerFacade.processEnableAccount(userId_enableAccount);
		return responseMessage.changeStatus("OK");
	}

	@RequestMapping(value = "/resetpassword", method = RequestMethod.GET)
	public String getResetPassowrd(@RequestParam(required = false) final String token, final HttpServletRequest request,
			final Model model) throws CMSItemNotFoundException
	{
		...
		storeCmsPageInModel(model, getContentPageForLabelOrId(RESETPASSWORD_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(RESETPASSWORD_PAGE));

		return "pages/ezg/account/resetPasswordPage";
	}

	@RequestMapping(value = "/resetpassword", method = RequestMethod.POST)
	@ResponseBody
	public ResponseMessage resetPassowrd(@RequestParam(required = false) final String token,
			@RequestParam(required = false) final String password, final HttpServletRequest request, final Model model)
					throws CMSItemNotFoundException
	{
		final ResponseMessage responseMessage = new ResponseMessage();
		if (StringUtils.isBlank(password) || StringUtils.length(password) < 8 || StringUtils.length(password) > 255
				|| !password.matches(
						"([0-9].*([a-zA-Z].*[~!@#$%^&*()+=|{}':;',\\[\\].<>/?~]|[~!@#$%^&*()+=|{}':;',\\[\\].<>/?~].*[a-zA-Z])|[a-zA-Z].*([0-9].*[~!@#$%^&*()+=|{}':;',\\[\\].<>/?~]|[~!@#$%^&*()+=|{}':;',\\[\\].<>/?~].*[0-9])|[~!@#$%^&*()+=|{}':;',\\[\\].<>/?~].*([0-9].*[a-zA-Z]|[a-zA-Z].*[0-9]))"))
		{
			responseMessage.changeStatus("Error");
			responseMessage.addInfo("password", "Password does not meet minimum requirements.");
		...
		else
		{
			try
			{
				customerFacade.updatePassword(token, password);
			}
			catch (final TokenInvalidatedException e)
			{
				LOG.error("Token for resetting password is invalid.");
				responseMessage.setMessage("Token is invalid.");
				return responseMessage.changeStatus("Error");
			}
			catch (final IllegalArgumentException e)
			{
				LOG.error("Token for resetting password is expired.");
				responseMessage.setMessage("Token is expired.");
				return responseMessage.changeStatus("Error");
			}
		}
		return responseMessage.changeStatus("OK");
