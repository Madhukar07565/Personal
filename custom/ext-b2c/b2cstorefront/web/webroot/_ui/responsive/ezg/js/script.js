...
$(document).ready(function(e) {
					...
					$('#mainNav').find('li').each(function(i, e){
						$(this).click(function(index){
							$('#mainNav').find('li').removeClass('sel');
							$('#mainNav').find('li').eq(index).addClass('sel');
							for (var i = 0; i < 5; i++) {
							...
							if (!$("#primary_navigation").exists()){
								...
								var url = ACC.config.encodedContextPath+"?tabName="+value;
								window.location=(url);
							}
...
//							});
					...
					var showHideOption;
					$(".moreFlightOpton1,.moreHotelOpton1").each(function(){
						$(this).click(function(){
							$(this).toggleClass("active");
							if($(this).hasClass("moreFlightOpton1"))
							{
								$(this).parent().next(".flightOptionsSearchPanel").slideToggle();
							}else{
								$(this).parent().next(".hotelOptionsSearchPanel").slideToggle();
							}
							if ($(this).hasClass('active')) {
								showHideOption = 1;
								$(this).find(".plusMinus1").html(" - ");
							} else {
								$(this).find(".plusMinus1").html(" + ");
							}
							if (showHideOption == 1)
					...
					if (jQuery().typed) {
						$("#autoTypeText").typed(
								{
									strings : [ '#family', '#shopper',
											'#honeymoon', '#budget traveller',
											'#first time abroad',
											'#photographer' ],
									typeSpeed : 0,
									backSpeed : 0,
					...
					// Function to stick the secondry nav to the top as user
					// scrolls for homepage
					if ($("#primary_navigation").exists()) {
											...
											if ($(window).scrollTop() > stickyRibbonTop) {
												$('#primary_navigation')
														.addClass(
																'primaryNavfixTop')
														.removeClass(
																'primaryNavigation_fixed');

												if ($(".globalSrcExpContr").is(
														":visible")) {
													$(
					...
					// Function to stick the secondry nav to the top as user
					// scrolls for all other pages
					if ($("#primary_navigation_inner").exists()) {
						var stickyRibbonTop = $('#primary_navigation_inner')
								.offset().top;
						// $('#primary_navigation_container').show();
						$(window)
								.scroll(
					...
					// auto complete functions//modify by aizijie
					if ($().autocomplete) {
						$(".autoSuggestText")
								.autocomplete({
									source : function( request, response ) {
										...
										var responseData;
										 $.getJSON(url+"/search/autocomplete/test", {term:request.term},response);
									},
									focus: function( event, ui ) {return false},
									open:function(){$(this).autocomplete("widget").css({ "width" : 310 });}
								});
							}

					/* Autocomplete function for overlays */
					if ($().autocomplete) {
						$("#holidayGoingTo,#hotelName,#hotelLoc")
								.focus(
										function() {
											$(
													"#holidayGoingTo,#hotelName,#hotelLoc")
					...
					var wishListItems = [ 'Mumbai, India (BOM)',
							'New Delhi, India (DEL)', 'Bangalore, India (BLR)',
							'Daman, India ', 'Chennai, India', 'Australia',
					...
					$('#travel .ritPnl .heading').on(
							'click',
							function(e) {
								$(this).parent().find('.header').closest(
										".contContainer").slideToggle();
					...
					if ($().datepicker) {
						// For Dual Calendar
						$("input[type='text'].dateField").datepicker({
							numberOfMonths : 2,
							minDate : 0,
							dateFormat : "yy-mm-dd",
					...
					$('.tabs .tabLinks_01 a').on(
							'click',
							function(e) {
								var currentAttrValue1 = $(this).attr('href');
								$('.tabs ' + currentAttrValue1).show()
										.siblings().hide();
								$(this).parent('li').addClass('sel').siblings()
										.removeClass('sel');
								e.preventDefault();
					...
					if ($('.miniTable').exists()) {
						$(document).scroll(
								function() {
									...
									$(".miniTable").width(
											$(".white_content_inner_all").width());
								});

						// Dynamic table width assignment
						var container = $(".white_content_inner_all").width();
					...
					$("table")
							.on(
									"click",
									".closeBtn_popup_small",
									function(event) {
										var mainTable = $(this).parent()
												.index() + 1;
										var count = $(".closeBtn_popup_small")
												.parent().length;
										if (count > 2) {
											if (count == 3) {
												$(".closeBtn_popup_small")
											...
											$("td", event.delegateTarget)
													.remove(
															":nth-child("
																	+ mainTable
																	+ ")");
											$("table.miniTable td").remove(
													":nth-child(" + mainTable
															+ ")");
										}
									});
					...
					if ($().slider) {
						...
						$("#departure").val(
								$("#slider-departure").slider("values", 0)
										+ ":00 -"
										+ $("#slider-departure").slider(
												"values", 1) + ":00");
						...
						$("#departure_1").val(
								$("#slider-departure_1").slider("values", 0)
										+ ":00 -"
										+ $("#slider-departure_1").slider(
												"values", 1) + ":00");
						...
						$("#departure_4").val(
								$("#slider-departure_4").slider("values", 0)
										+ ":00 -"
										+ $("#slider-departure_4").slider(
												"values", 1) + ":00");
						...
						$("#departure_6").val(
								$("#slider-departure_6").slider("values", 0)
										+ ":00 -"
										+ $("#slider-departure_6").slider(
												"values", 1) + ":00");
						$("#slider-departure1")
						...
						$("#departure1").val(
								$("#slider-departure1").slider("values", 0)
										+ ":00 -"
						...
						$("#return").val(
								$("#slider-return").slider("values", 0)
										+ ":00 -"
										+ $("#slider-return").slider("values",
												1) + ":00");
						...
						$("#flightreturn1").val(
								$("#slider-flightreturn1").slider("values", 0)
										+ ":00 -"
										+ $("#slider-flightreturn1").slider(
												"values", 1) + ":00");
						...
						$("#return1").val(
								$("#slider-return1").slider("values", 0)
										+ ":00 -"
										+ $("#slider-return1").slider("values",
												1) + ":00");
						...
						$("#amount-port").val(
								$("#slider-range-port").slider("values", 0)
										+ " - "
										+ $("#slider-range-port").slider(
												"values", 1));
						...
						$("<span class='rupee'>Rs.</span>").insertBefore(
								"#priceChange");
						$("#priceChange").val(
						...
						$("#amount768").val(
								$("#slider-range768").slider("values", 0)
										+ " - "
										+ $("#slider-range768").slider(
												"values", 1));
						...
						$("#camount768").val(
								$("#slider-crange768").slider("values", 0)
										+ " - "
										+ $("#slider-crange768").slider(
												"values", 1));
						...
						$("#amt768").val(
								$("#slider-rang768").slider("values", 0)
										+ " - "
										+ $("#slider-rang768").slider("values",
												1));
						...
						$("#day").val(
								$("#slider-day").slider("values", 0) + "D - "
										+ $("#slider-day").slider("values", 1)
										+ "D");
						...
						$("#day1").val(
								$("#slider-day1").slider("values", 0) + "D - "
										+ $("#slider-day1").slider("values", 1)
										+ "D");
						...
						$("#amountBudget").val(
								$("#slider-range-budget").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget").slider(
												"values", 1));
						...
						$("#amountBudget1").val(
								$("#slider-range-budget1").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget1").slider(
												"values", 1));
						...
						$("#amountBudget2").val(
								$("#slider-range-budget2").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget2").slider(
												"values", 1));
						...
						$("#amountBudget3").val(
								$("#slider-range-budget3").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget3").slider(
												"values", 1));
						...
						$("#amountBudget4").val(
								$("#slider-range-budget4").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget4").slider(
												"values", 1));
						...
						$("#amountBudget5").val(
								$("#slider-range-budget5").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget5").slider(
												"values", 1));
						...
						$("#amountBudget6").val(
								$("#slider-range-budget6").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget6").slider(
												"values", 1));
						...
						$("#amountBudget7").val(
								$("#slider-range-budget7").slider("values", 0)
										+ " - "
										+ $("#slider-range-budget7").slider(
												"values", 1));

						if ($("#number1_flight").exists()) {
							begin_animation()
						}
					...
					function animate() {
						_animate = setInterval(function() {
							$('#slideshow > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow');
						}, 6000);
					}
					...
					function animate2() {
						_animate2 = setInterval(function() {
							$('#slideshow2 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow2');
						}, 43000);
					}
					...
					function animate3() {
						_animate3 = setInterval(function() {
							$('#slideshow3 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow3');
						}, 37000);
					}
					...
					var active4 = 1;
					function animate4() {
						_animate4 = setInterval(
								function() {
									$('#slideshow4 > div:first').fadeOut(1000)
					...
					function animate5() {
						_animate5 = setInterval(function() {
							$('#slideshow5 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow5');
						}, 29000);
					}
					...
					function animate6() {
						_animate6 = setInterval(function() {
							$('#slideshow6 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow6');
						}, 15000);
					}
					...
					function animate7() {
						_animate7 = setInterval(function() {
							$('#slideshow7 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow7');
						}, 21000);
					}
					...
					function animate8() {
						_animate8 = setInterval(function() {
							$('#slideshow8 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow8');
						}, 23000);
					}
					...
					function animate9() {
						_animate9 = setInterval(function() {
							$('#slideshow9 > div:first').fadeOut(1000).next()
									.fadeIn(200).end().appendTo('#slideshow9');
						}, 27000);
					}
					...
					var active10 = 1;
					function animate10() {
						_animate10 = setInterval(
								function() {
									$('#slideshow10 > div:first').fadeOut(1000)
											.next().fadeIn(200).end().appendTo(
													'#slideshow10');
								}, 15000);
					}
					...
					var active11 = 1;
					function animate11() {
						_animate11 = setInterval(
								function() {
									$('#slideshow11 > div:first').fadeOut(1000)
											.next().fadeIn(200).end().appendTo(
													'#slideshow11');
								}, 39000);
					}
					...
					var active12 = 1;
					function animate12() {
						_animate12 = setInterval(
								function() {
									$('#slideshow12 > div:first').fadeOut(1000)
											.next().fadeIn(200).end().appendTo(
													'#slideshow12');
								}, 11000);
					}
					...
					//set slider images
					if( $('img.mT20')){
						var prevButton = $('img.mT20:eq(0)');
						var nextButton = $('img.mT20:eq(1)')
						if(typeof(ractiveTopFlight) != "undefined"){
							if(ractiveTopFlight.resultList){
								prevButton.attr('src', ractiveTopFlight.resultList.get('prevButton'));
								nextButton.attr('src', ractiveTopFlight.resultList.get('nextButton'));
							}
						}
					}
					...
					var active13 = 1;
					function animate13() {
						_animate13 = setInterval(
								function() {
									$('#slideshow13 > div:first').fadeOut(1000)
											.next().fadeIn(200).end().appendTo(
													'#slideshow13');
								}, 19000);
					}
...
//					$("li[id^='airlineTab']").on('click', function (e) {
////						alert($("li[id^='airlineTab']").length);
//						//ractiveFlight.submitForm('facet');
//						$("div[id^='airlineTabResults']").hide();
...
//					});
					...
					if ($('#nav-anchor').exists()) {
						...
						var aChildren = $("nav.secNav li").children(); // find
																		// the a
																		// children
																		// of
																		// the
						...
						for (var i = 0; i < aChildren.length; i++) {
							...
							aArray.push(ahref);
						} // this for loop fills the aArray with attribute
							// href values
								...
								.scroll(
										function() {
											...
											for (var i = 0; i < aArray.length; i++) {
												var theID = aArray[i];
												var divPos = $(theID).offset().top; // get
																					// the
																					// offset
																					// of
																					// the
					...
					/*
					 * if($().calendario) { var cal = $( '#calendar1'
					 * ).calendario( { onDayClick : function( $el, $contentEl,
					 * dateProperties ) { for( var key in dateProperties ) {
					 * console.log( key + ' = ' + dateProperties[ key ] ); } },
					 * caldata : codropsEvents } ), $month = $( '#custom-month1'
					 * ).html( cal.getMonthName() ), $year = $( '#custom-year1'
					 * ).html( cal.getYear() );
					 ...
					 * cal.gotoNow( updateMonthYear ); } ); function
					 * updateMonthYear() { $month.html( cal.getMonthName() );
					 * $year.html( cal.getYear() ); } } if($().calendario) { var
					 * cal = $( '#calendar2' ).calendario( { onDayClick :
					 * function( $el, $contentEl, dateProperties ) { for( var
					 * key in dateProperties ) { console.log( key + ' = ' +
					 * dateProperties[ key ] ); } }, caldata : codropsEvents } ),
					 ...
					 * updateMonthYear() { $month.html( cal.getMonthName() );
					 * $year.html( cal.getYear() ); } } if($().calendario) { var
					 * cal = $( '#calendar3' ).calendario( { onDayClick :
					 * function( $el, $contentEl, dateProperties ) { for( var
					 * key in dateProperties ) { console.log( key + ' = ' +
					 * dateProperties[ key ] ); } }, caldata : codropsEvents } ),
					 ...
					 * updateMonthYear() { $month.html( cal.getMonthName() );
					 * $year.html( cal.getYear() ); } } if($().calendario) { var
					 * cal = $( '#calendar4' ).calendario( { onDayClick :
					 * function( $el, $contentEl, dateProperties ) { for( var
					 * key in dateProperties ) { console.log( key + ' = ' +
					 * dateProperties[ key ] ); } }, caldata : codropsEvents } ),
					 ...
					 * updateMonthYear() { $month.html( cal.getMonthName() );
					 * $year.html( cal.getYear() ); } } if($().calendario) { var
					 * cal = $( '#calendar5' ).calendario( { onDayClick :
					 * function( $el, $contentEl, dateProperties ) { for( var
					 * key in dateProperties ) { console.log( key + ' = ' +
					 * dateProperties[ key ] ); } }, caldata : codropsEvents } ),
					 ...
					 * updateMonthYear() { $month.html( cal.getMonthName() );
					 * $year.html( cal.getYear() ); } } if($().calendario) { var
					 * cal = $( '#calendar6' ).calendario( { onDayClick :
					 * function( $el, $contentEl, dateProperties ) { for( var
					 * key in dateProperties ) { console.log( key + ' = ' +
					 * dateProperties[ key ] ); } }, caldata : codropsEvents } ),
...
//	}
//}
function popupwin(light, lightInner, fade, callback) {
	ACC.account.callback=callback;
...
function showHolidayPackage(obj, id) {
	...
	var imgCount = (document.getElementById('thumb1')
...
/*
 * arrVar = 0; depVar = 0; function selectItem(id1, id2,flStat){ if(flStat ==
 * "arr") { aa = document.getElementById(id1); bb =
 * document.getElementById(id2); if(aa.style.display == 'none') { if(arrVar ==
 * 0) { aa.style.display = 'block'; bb.style.backgroundColor ="#eeeeee"; $("#" +
 * id2).addClass("sel"); arrVar = 1; } else{ for(i=1;i<=4;i++) {
 * document.getElementById(flStat + "Tick" + i).style.display = "none";
 * document.getElementById(flStat + "FlightPlan" + i +
 * "_inner").style.backgroundColor = "transparent"; $("#" + flStat +
 * "FlightPlan" + i + "_inner").removeClass("sel"); } aa.style.display =
 * 'block'; bb.style.backgroundColor ="#eeeeee"; $("#" + id2).addClass("sel");
 * arrVar = 1; } } else{ aa.style.display = 'none'; bb.style.backgroundColor =
 * "transparent"; $("#" + id2).removeClass("sel"); arrVar = 0; } }
 * 
 * if(flStat == "dep") { aa = document.getElementById(id1); bb =
 * document.getElementById(id2); if(aa.style.display == 'none') { if(depVar ==
 * 0) { aa.style.display = 'block'; bb.style.backgroundColor ="#eeeeee"; $("#" +
 * id2).addClass("sel"); depVar = 1; } else{ for(i=1;i<=4;i++) {
 * document.getElementById(flStat + "Tick" + i).style.display = "none";
 * document.getElementById(flStat + "FlightPlan" + i +
 * "_inner").style.backgroundColor = "transparent"; $("#" + flStat +
 * "FlightPlan" + i + "_inner").removeClass("sel"); } aa.style.display =
 * 'block'; bb.style.backgroundColor ="#eeeeee"; $("#" + id2).addClass("sel");
 * arrVar = 1; } } else{ aa.style.display = 'none'; bb.style.backgroundColor
 * ="transparent"; $("#" + id2).removeClass("sel"); depVar = 0; } }
 * 
 * if(document.getElementById("depTick1").style.display == "block" &&
 * document.getElementById("arrTick1").style.display == "none") {
 ...
 * 
 * if(document.getElementById("depTick1").style.display == "none" &&
 * document.getElementById("arrTick1").style.display == "block") {
 * document.getElementById("show_holiday_panel_top").style.display = "block";
...
function showRate(rate) {
	...
	if (rate == "dep") {
		$("#show_holiday_panel_top,#depFlightSel,#flightAddIcon,#arrFlightSel")
				.show();
...
function hideRate_01() {
	...
	if ($("#del_bom").is(":visible")) {
		$(
				"#del_bom,#show_holiday_panel_top,#depFlightSel,#flightAddIcon,#arrFlightSel")
				.hide();
		$("#flightPrice").html('');
	} else {
		$(
...
function begin_animation() {
	var hotelnumber = parseInt($('#flightnumber').text());
	var amount1 = parseInt($('#amount1_flight').text());
	...
	$({
		...
		step : function(now) {
			$("#number1_flight").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
	$({
		...
		step : function(now) {
			$("#number2_flight").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
...
function searchFlight(obj) {
	...
	case "oneway":
		...
		// document.getElementById("multiwaySearchPanel_2").style.display =
		// "none";
		...
		$(
				"#multiwaySearchParameters3, #multiwaySearchParameters4, #multiwaySearchParameters5, #multiwaySearchParameters6")
				.hide();
...
function begin_animation_holidays() {
	...
	$({
		...
		step : function(now) {
			$("#number1_holiday").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
	$({
		...
		step : function(now) {
			$("#number2_holiday").text(Math.round(now) + ',')
		},
		duration : 1000,
		easing : "linear"
	});
	$({
		...
		step : function(now) {
			$("#number3_holiday").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
...
function begin_animation_hotels() {
	var hotelnumber = parseInt($('#hotelnumber').text());
	...
	$({
		...
		step : function(now) {
			$("#number1_hotel").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
	$({
		...
		step : function(now) {
			$("#number2_hotel").text(Math.round(now))
		},
		duration : 1000,
		easing : "linear"
	});
...
function showMoreInfo(id1, id2, id3) {
	...
	if ($("#" + id1).hasClass('active')) {
		...
		if ($("#multicity").is(":checked")) {
			if ($("#multiwaySearchParameters3").is(":hidden")
					&& $("#multiwaySearchParameters4").is(":hidden")
					&& $("#multiwaySearchParameters5").is(":hidden")) {
				showHidePanel = 0;
			}
		} else {
...
function showMoreInfoHotel(id1, id2, id3) {
	...
	if ((!$("#" + id1).hasClass('active')) && (child_Count < 3)
			&& ($("#noOfRooms").val() < 2) && !$("#addFlight1").is(":checked")
			&& !$("#addCar").is(":checked")) {
		showHidePanel = 0;
	}
...
function addCarFlight(option) {
	if (option == "car") {
		if ($("#carSearchPanel").is(":visible")) {
			...
			if ((!$("#addFlight1").is(":checked"))
					&& ($("#flightOptionsSearchPanel").is(":hidden"))
					&& ($("#noOfRooms").val() < 2)) {
				showHidePanel = 0;
			}
	...
	if (option == "Hotel") {
		if ($("#hotelStayPanel").is(":visible")) {
			...
			if ((!$("#addHotelStay").is(":checked"))
					&& ($("#hotelOptionsSearchPanel").is(":hidden"))
					&& ($("#noOfRooms").val() < 2)) {
				showHidePanel = 0;
			}
	...
	if (option == "flight") {
		if ($("#flightSearchPanel").is(":visible")) {
			...
			if ((child_Count < 3) && (!$("#addFlight1").is(":checked"))
					&& (!$("#addFlight1").is(":checked"))
					&& (!$("#moreFlightOpton1").hasClass("active"))) {
				showHidePanel = 0
			}
		...
		return this
				.each(function() {
					...
					if (contentlen > config.showChars) {
						...
						if (c.indexOf('<') >= 0) // If there's HTML don't
													// want to cut it
						{
							var inTag = false; // I'm in a tag?
							var bag = ''; // Put the characters to be shown
											// here
							var countChars = 0; // Current bag size
							var openTags = []; // Stack for opened tags, so I
							...
							for (var i = 0, r = 0; r <= config.showChars; i++) {
								if (content[i] == '<' && !inTag) {
									...
									tagName = content.substring(i + 1, content
											.indexOf('>', i));
									...
									if (tagName[0] == '/') {
										...
										// There are some nasty tags that don't
										// have a close tag like <br/>
										if (tagName.toLowerCase() != 'br') {
											openTags.unshift(tagName); // Add
																		// to
																		// start
																		// the
																		// name
								...
								if (inTag) {
									...
									if (countChars <= config.showChars) {
										bag += content.charAt(i); // Fix to ie
																	// 7 not
																	// allowing
																	// you to
																	// reference
									...
									{
										if (openTags.length > 0) // I have
																	// unclosed
																	// tags
										{
											// console.log('They were open
											// tags');
											// console.log(openTags);
											for (j = 0; j < openTags.length; j++) {
												// console.log('Cierro tag ' +
												// openTags[j]);
												bag += '</' + openTags[j] + '>'; // Close
																					// all
																					// tags
																					// that
																					// were
												...
												// You could shift the tag from
												// the stack to check if you end
												// with an empty stack, that
												// means you have closed all
												// open tags
											}
											break;
										}
						...
						$('.shortcontent p:last', $this)
								.css('margin-bottom', 0); // Remove bottom
															// margin on last
															// paragraph as it's
															// likely shortened
					}
...
function hideMultiwayOptions(i) {
	...
	if ($("#multiwaySearchParameters3").is(":hidden")
			&& $("#multiwaySearchParameters4").is(":hidden")
			&& $("#multiwaySearchParameters5").is(":hidden")
			&& $("#multiwaySearchParameters6").is(":hidden")) {
		showHidePanel = 0;
	}
	if (i != 5)
		mc = i++;
...
function showMoreMobileNumbers() {
	for (var i=2; i<=4; i++) {
		if ($("#mobileNumber_div" + i).css('display') == 'none') {
...
function clearAndHideMobileNumber(i) {
	// clear input box
	$("#mobileNumber_div" + i + " .mR10").val("");
	$("#mobileNumber_div" + i + " .selectContLargeInput").val("");
	// hide input box
	$("#mobileNumber_div" + i).hide();
}
...
$(".followButton").click(
		function() {
			($(".followButton").text() === "Follow") ? $(".followButton").text(
					"Unfollow") : $(".followButton").text("Follow");
		});

// Remove current tile
/*
 * $(".recycle_icon_wishlist").on("click", function(){ var currentTileId =
 * $(this).closest('.boxTile').attr('id'); console.log(currentTileId)
...
function createWishlistCategory() {
	...
	var wishlistBucketContent = '<article class="col_4 posR mB40 boxTile" id="wishListCont'
			+ wishlistItems
			+ '"><img src="images/wishlist/wishlist_australia.jpg" alt="Hotel" class="brdrWht" /><a href="javascript:void(0)"><div class="wl_innerContr"><h2 class="tC fcGrn tileHeader" id="tileHeader'
			+ wishlistItems
			+ '">'
			+ wishListValue
			+ '</h2><span class="fS30 tC dB defaultTxtColor">0</span></div></a><span class="tC dB wishlistIcons"> <span class="share_icon mR10"></span> <span class="edit_icon_wishlist mR10" onClick="editWishListName('
			+ wishlistItems
			+ ')"></span> <span class="recycle_icon_wishlist"></span> </span></article>';
...
/*
 * $(document).ready(function() { $(".edit_icon_wishlist").click(editField);
 * 
 * });
 */
...
function selectFlight(flightNo, path) {
	if (path == "arrival") {
		...
		document.getElementById('selectContainer1').innerHTML = '<span class="fL mR10"><img src="images/srp_flight/indigo_sml_logo.jpg" class="mB5" alt="Air India Logo" /><span class="fS12 fcWht dB">Multiple <br>airlines</span></span><span class="clearfix fL mTN5 dB"><span class="clearfix dB"><span class="fcWht fL TrvLocation">Mumbai<br /> 11.30</span><span class="flight_icon_sml fL mTB2LR10"></span><span class="fcWht fL TrvLocation">Delhi<br /> 4.35</span></span><span class="fS11 fcWht dB">3h 5m  | 1 stop </span><span class="fS11 fcWht dB">Reaches 24 Jul, Sun</span></span>';
		document.getElementById("selectContainer1").style.display = "block";
		document.getElementById("addSFlight").style.display = "none";
		document.getElementById("priceBox").style.display = "block";
		document.getElementById('dirImg').style.display = "block";
	...
	if (path == "return") {
		...
		document.getElementById('selectContainer2').innerHTML = '<span class="fL mR10"><img src="images/srp_flight/indigo_sml_logo.jpg" class="mB5" alt="Air India Logo" /><span class="fS12 fcWht dB">Multiple <br>airlines</span></span><span class="clearfix fL mTN5 dB"><span class="clearfix dB"><span class="fcWht fL TrvLocation">Mumbai<br /> 11.30</span><span class="flight_icon_sml fL mTB2LR10"></span><span class="fcWht fL TrvLocation">Delhi<br /> 4.35</span></span><span class="fS11 fcWht dB">3h 5m  | 1 stop </span><span class="fS11 fcWht dB">Reaches 24 Jul, Sun</span></span>';
	...
	if ((document.getElementById('selectContainer1').innerHTML != '')
			&& (document.getElementById('selectContainer2').innerHTML != ''))
...
function addOneWayFlight(no) {
	document.getElementById("depFlightSel").innerHTML = '<span class="fL mR10"><img src="images/srp_flight/indigo_sml_logo.jpg" class="mB5" alt="Air India Logo" /><span class="fS12 fcWht dB">Multiple <br>airlines</span></span><span class="clearfix fL mTN5 dB"><span class="clearfix dB"><span class="fcWht fL TrvLocation">Mumbai<br /> 11.30</span><span class="flight_icon_sml fL mTB2LR10"></span><span class="fcWht fL TrvLocation">Delhi<br /> 4.35</span></span><span class="fS11 fcWht dB">3h 5m  | 1 stop </span><span class="fS11 fcWht dB">Reaches 24 Jul, Sun</span></span> ';
	document.getElementById("priceBox").style.display = "block";
}
...
var compCruises = 0;
/*
 * function checkThis(hId){ if(document.getElementById("holiday" + hId).checked ==
 * true && addedIds.indexOf(hId) == -1) {
 * document.getElementById("show_holiday_panel_top").style.display = "block";
 * document.getElementById("searchedResult" + compHolidays).innerHTML = '<div
 * class="fL mR10 mT5 mB5"> <img src="images/common/hotel_thumb.png" alt="Hotel
 * Thumb" /> </div><div class="fL"><div class="fcWht">' +
 * document.getElementById("holidayDest" + hId).innerHTML + '</div><div
 * class="fS11 fcWht">7 Days / 6 Nights</div><div class="fS11 fcWht mB5">Port
 * Blair | Havelock | Neil Island</div><div class="clearfix"><div class="fS22
 * fW400 fL fcWht"><span class="rupee">Rs</span>86,000</div></div></div>';
 * if(compHolidays > 1) document.getElementById("compareButton").style.display =
 * "block"; document.getElementById("compareCount").innerHTML = compHolidays;
 * addedIds[k] = hId; compHolidays++; k++; } }
 */
function checkThis1(hId) {
	if (document.getElementById("holiday" + hId).checked == true) {
		...
		a.innerHTML = '<div class="fL mR10 mT5 mB5"> <img src="images/activities/activity1.jpg" alt="Hotel Thumb" /> </div><div class="fL"><div class="fcWht">'
				+ document.getElementById("holidayDest" + hId).innerHTML
				+ '</div><div class="fS11 fcWht">7 Days / 6 Nights</div><div class="fS11 fcWht mB5">Port Blair | Havelock | Neil Island</div><div class="clearfix"><div class="fS22 fW400 fL fcWht"><span class="rupee">Rs</span>86,000</div></div></div>';
		if (compHolidays > 0)
...
function checkThis(hId) {
	if (document.getElementById("holiday" + hId).checked == true) {
		...
		a.innerHTML = '<div class="fL mR10 mT5 mB5"> <img src="images/common/hotel_thumb.png" alt="Hotel Thumb" /> </div><div class="fL"><div class="fcWht">'
				+ document.getElementById("holidayDest" + hId).innerHTML
				+ '</div><div class="fS11 fcWht">7 Days / 6 Nights</div><div class="fS11 fcWht mB5">Port Blair | Havelock | Neil Island</div><div class="clearfix"><div class="fS22 fW400 fL fcWht"><span class="rupee">Rs</span>86,000</div></div></div>';
		if (compHolidays > 1)
...
function compareCruise(cruiseId) {
	if (document.getElementById("cruise" + cruiseId).checked == true) {
		...
		a.innerHTML = '<div class="fL mR10 mT5 mB5"> <img src="images/common/cruise_thumb.jpg" alt="Cruise Thumb" /> </div><div class="fL"><div class="fcWht">'
				+ document.getElementById("cruiseName" + cruiseId).innerHTML
				+ '</div><div class="fcWht fS11">4 star</div><div class="clearfix"><div class="fS22 fW400 fL fcWht"><span class="rupee">Rs</span>86,000</div></div></div>';
		if (compCruises > 1)
			document.getElementById("compareButton").style.display = "block";
		compCruises++;
		...
		function(e) {
			...
			if ($(this).hasClass('active')) {
				$(this).find('img').attr('src',
						ezgImagesPath + '/common/down-arrow-wht.png');
			} else {
				$(this).find('img').attr('src',
						ezgImagesPath + '/common/right-arrow-wht.png');
			}
		});
...
function showtooltip(parentId, tooltipId) {
	...
	if (parentId == "cardNumber") {
		var left = ($("#" + parentId).offset().left) - parentElementWidth + 5
				+ 'px';
		var top = ($("#" + parentId).offset().top) + parentElementHeight + 15
				+ 'px';
		...
		var left = ($("#" + parentId).offset().left) - parentElementWidth - 100
				+ 'px';
		var top = ($("#" + parentId).offset().top) + parentElementHeight + 15
				+ 'px';
	} else {
		var left = ($("#" + parentId).offset().left) - parentElementWidth + 100
				+ 'px';
		var top = ($("#" + parentId).offset().top) + parentElementHeight + 25
				+ 'px';
	}
 ...
 * $("#"+curContr).slideUp('slow'); $("#"+nextContr).slideDown('slow');
 * document.getElementById(curHrdr).onclick = fn_step1; $('html,
 * body').animate({scrollTop: '0px'},500); }
 * 
 * function fn_step1(){ $("#header1").addClass('active');
 ...
 * $("#header1").next(".accordion_stepForm
 * .contContainer").slideDown("slow").siblings(".accordion_stepForm
 * .contContainer:visible").slideUp("slow"); $('html, body').animate({scrollTop:
 * '0px'},500); }
 * 
 ...
 * $("#"+nextHrdr).addClass('active').removeClass('noImg');
 * $("#"+curContr).slideUp('slow'); $("#"+nextContr).slideDown('slow');
 * document.getElementById(curHrdr).onclick = fn_step2; $('html,
 * body').animate({scrollTop: '0px'},500); }
 * 
 * function fn_step2(){ $("#header2").addClass('active');
 ...
 * $("#header2").next(".accordion_stepForm
 * .contContainer").slideDown("slow").siblings(".accordion_stepForm
 * .contContainer:visible").slideUp("slow"); $('html, body').animate({scrollTop:
 * '0px'},500); }
 * 
 ...
 * $("#"+nextHrdr).addClass('active').removeClass('noImg');
 * $("#"+curContr).slideUp('slow'); $("#"+nextContr).slideDown('slow');
 * document.getElementById(curHrdr).onclick = fn_step3; $('html,
 * body').animate({scrollTop: '0px'},500); }
 * 
 * function fn_step3(){ $("#header3").addClass('active');
 ...
 * $("#header3").next(".accordion
 * .contContainer").slideDown("slow").siblings(".accordion_stepForm
 * .contContainer:visible").slideUp("slow"); $('html, body').animate({scrollTop:
 * '0px'},500); } function step4(curHrdr, nextHrdr, curContr, nextContr){
 * $("#"+curHrdr).removeClass('active noImg');
 * $("#"+nextHrdr).addClass('active').removeClass('noImg');
 * $("#"+curContr).slideUp('slow'); $("#"+nextContr).slideDown('slow');
 * document.getElementById(curHrdr).onclick = fn_step4; $('html,
 * body').animate({scrollTop: '0px'},500); }
 * 
 * function fn_step4(){ $("#header4").addClass('active');
...
$('.diyContr header span.heading').on(
		'click',
		function(e) {
			...
			if ($(this).hasClass('sel')) {
				$(this).find('img').attr('src',
						ezgImagesPath + '/common/expand.png');
			} else {
				$(this).find('img').attr('src',
						ezgImagesPath + '/common/collapse.png');
			}
		});
...
function editCurrentField(markupId) {
	...
	document.getElementById("markup" + markupId).innerHTML = '<input id="markupEdit'
			+ markupId
			+ '" type="text" value="" class="small"><span class="tick_icon mL2" onclick="editFieldValue('
			+ markupId + ');"></span>';
...
function editFieldValue(id) {
	document.getElementById("markup" + id).innerHTML = '<h4 class="fL"><span id="markupVal'
			+ id
			+ '">'
			+ document.getElementById("markupEdit" + id).value
			+ '</span></h4><span class="edit_icon mL5" onClick="editCurrentField('
			+ id + ');"></span>';
}
...
function nextAro() {
	...
	$('.detail_indContr').hide(); // Hide any visible tooltips when clicked
									// next arrow
}
function prevAro() {
	slideNumber--;
...
function selectMultiFlight(flightLeg) {
	...
	if ((leg1Added == 1) && (leg2Added == 1) && (leg3Added == 1)
			&& (leg4Added == 1))
		document.getElementById("selectBooking").style.display = "block";
	else
		document.getElementById("selectBooking").style.display = "none";
...
$('.shopCardHrdr').on(
		'click',
		function(e) {
			$(this).nextAll('.shopCardContr').slideToggle();
			$(this).toggleClass('sel');
			if ($(this).hasClass('sel')) {
				$(this).find('img').attr('src',
						ezgImagesPath + '/common/expand.png');
...
function Decrement() {
	...
	document.getElementById("timerText").innerHTML = currentMinutes + ":"
			+ currentSeconds + " Min"; // Set the element id you need the time
										// put into.
	if (secs !== -1)
		setTimeout('Decrement()', 1000);
}

// modified by aizijie 2016-06-23 start
var roomsAdded = 1;
function showRooms() {
	...
	if ((child_Count < 3) && (!$("#addCar").is(":checked"))
			&& (!$("#addFlight1").is(":checked"))
			&& (!$("#moreFlightOpton1").hasClass("active")) && (rooms < 2)) {
		showHidePanel = 0
	}
	...
	if (rooms < roomsAdded) {
		...
		for (r = 1; r <= rAdd; r++) {
			document.getElementById("room" + (roomsAdded + 1)).innerHTML = '<ul class="row mB10 mT15 clearfix">'
					+ '<li class="col_2">&nbsp;</li>'
					+ '<li class="col_2 pR0"> <span class="mT30 fL">Room '
					+ (roomsAdded + 1)
					+ ':</span></li>'
					+ '<li class="col_2 pL0"><label>Adult</label>'
					+ '<div class="adderOuter">'
					+ '<span  onclick="adultCountDec(\'hAdult_'
					+ (roomsAdded + 1)
					+ '\')" class="control addMinControl minusControl cursor fW600">-</span>'
					+ '<span id="hAdult_'
					+ (roomsAdded + 1)
					+ '" class="control innerTxtControl">1</span>'
					+ '<span onclick="adultCountInc(\'hAdult_'
					+ (roomsAdded + 1)
					+ '\')" class="control addMinControl plusControl cursor fW600">+</span> </div>'
					+ '<input type="hidden" class="w100pcnt" maxlength="3" name="adultNo" value="1"><span class="fS11 fL w100pcnt">12+ yrs</span></li>'
					+ '<li class="col_2 pL0"><label>Children</label><div class="adderOuter"> '
					+ '<span class="control addMinControl minusControl cursor fW600"  onclick="childCountDec(\'hChild_'
					+ (roomsAdded + 1)
					+ '\')">-</span> '
					+ '<span class="control innerTxtControl" id="hChild_'
					+ (roomsAdded + 1)
					+ '">0</span>'
					+ '<span class="control addMinControl plusControl cursor fW600" onclick="childCountInc(\'hChild_'
					+ (roomsAdded + 1)
					+ '\')">+</span>'
					+ '</div><input type="hidden" class="w100pcnt" maxlength="3" name="childNo" value="0"><span class="fS11 fL w100pcnt">2 - 12 yrs</span> </li> </ul>';
			roomsAdded++;
		}
	}
}
var initRoomNum = "${index}";
function showRooms_h() {
...
function adultCountInc(id, cntr) {
	...
	if (adult_Count < 9) {
		adult_Count++;
		document.getElementById(id).innerHTML = adult_Count;
		$("#" + id).parent().next("input").val(adult_Count);
	}
}
function adultCountDec(id, cntr) {
	adult_Count = document.getElementById(id).innerHTML;
...
function childCountInc(id, cntr) {
	...
	if (child_Count < 9) {
		child_Count++;
		document.getElementById(id).innerHTML = child_Count;
		$("#" + id).parent().next("input").val(child_Count);
	}
}
function childCountDec(id, cntr) {
	child_Count = document.getElementById(id).innerHTML;
...
// modified by aizijie 2016-06-23 end
function addChild(s) {
	document.getElementById("child" + s + "_" + child_Count).innerHTML = '<label>Child '
			+ child_Count
			+ '</label><select class="w75"><option value="Age">Age</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select>';
...
/*
 * $(".globalSearchBG").click( function(){ if( $("#mainSearchWizard").height() >
 * 320){
 *  } else { $(".globalSrcExpContr").hide(); } });
 */
...
function creatCategory() {
	...
	// $( '<div class="wishlist_List">' + categoryValue + ' </div> '
	// ).insertBefore($("#createActionCont"));
	$('<option selected="selected">' + categoryValue + ' </option> ').appendTo(
			$("#wishListCategorySelect"));
		...
		$({
			number : 1000
				...
				},
				{
					...
					easing : 'linear',
					step : function() {
						var updatedNum = $('#' + id4).text(
								parseFloat(Math.ceil(this.number * 100) / 100)
										.toFixed(0));
						var formattedNumber = $('#' + id4).text().replace(
								/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
...
function deleteSelect(id) {
	document.getElementById("selectFlightLeg" + id).innerHTML = '<div id="addFlightLeg1" class="addEurail_ActionPnl">Add leg '
			+ id + '</div>';
	if (id == 4) {
		leg4Added = 0;
...
/*
 * var leg = 1; leg1Added = leg2Added = 0;
 * 
 * function selectFlight(flightLeg,row){
 * document.getElementById('shopping_cart_panel').style.display = 'block';
 * $(".footerContactDetail").addClass("activated");
 * $("#shoppingCartTab").addClass('sel'); if(flightLeg == 1){
 * document.getElementById("selectFlightLeg1").innerHTML = '<span class="fL
 * mR10"><img src="images/srp_flight/indigo_sml_logo.jpg" class="mB5" alt="Air
 * India Logo" /><span class="fS12 fcWht dB">Multiple <br>airlines</span></span><span
 * class="clearfix fL mTN5 dB"><span class="clearfix dB"><span class="fcWht fL
 * TrvLocation">Mumbai<br /> 11.30</span><span class="flight_icon_sml fL
 * mTB2LR10"></span><span class="fcWht fL TrvLocation">Delhi<br /> 4.35</span></span><span
 * class="fS11 fcWht dB">3h 5m | 1 stop </span><span class="fS11 fcWht
 * dB">Reaches next day</span><span class="mT5 fR"><img src="images/bin.png"
 * alt="delete" onclick="deleteSelect2(1)" /></span></span>'; leg1Added = 1;
 * 
 * for(i=1;i<4;i++) {if(i== row)
 * document.getElementById("selflight"+row).style.display='block'; else
 * document.getElementById("selflight"+i).style.display='none'; } } if(flightLeg ==
 * 2){ document.getElementById("selectFlightLeg2").innerHTML = '<span class="fL
 * mR10"><img src="images/srp_flight/indigo_sml_logo.jpg" class="mB5" alt="Air
 * India Logo" /><span class="fS12 fcWht dB">Multiple <br>airlines</span></span><span
 * class="clearfix fL mTN5 dB"><span class="clearfix dB"><span class="fcWht fL
 * TrvLocation">Mumbai<br /> 11.30</span><span class="flight_icon_sml fL
 ...
 * class="fS11 fcWht dB">3h 5m | 1 stop </span><span class="fS11 fcWht
 * dB">Reaches next day</span><span class="mT5 fR"><img src="images/bin.png"
 * alt="delete" onclick="deleteSelect2(2)" /></span></span>'; leg2Added = 1;
 * document.getElementById("selectBooking").style.display = "block";
 * 
 * for(i=11;i<14;i++) {if(i== row)
 * document.getElementById("selflight"+row).style.display='block'; else
 * document.getElementById("selflight"+i).style.display='none'; }
 *  } document.getElementById("totalFare").innerHTML = (300*leg); if((leg1Added ==
 * 1) && (leg2Added == 1))
 * document.getElementById("selectBooking").style.display = "block"; else
 * document.getElementById("selectBooking").style.display = "none"; leg++;
 * if((leg1Added == 1) || (leg2Added == 1)) {
 * document.getElementById("workselect").style.display = "table";
 * document.getElementById("shopping_cart_panel").style.display = "block"; }
 * else { document.getElementById("workselect").style.display = "none";
 * document.getElementById('shopping_cart_panel').style.display = "none"; }
 * leg++; }
 * 
 * 
 * function deleteSelect2(id) {
 * document.getElementById("selectFlightLeg"+id).innerHTML='<div
 * id="addFlightLeg1" class="addFlight_ActionPnl">Add leg '+id+' </div>'; if(id
 * ==1) { leg1Added = 0; for(i=1;i<4;i++) {
 * document.getElementById("selflight"+i).style.display='none'; } } if(id ==2) {
 * leg2Added = 0; for(i=11;i<14;i++) {
 * document.getElementById("selflight"+i).style.display='none'; } }
 * 
 * leg--; document.getElementById("totalFare").innerHTML = (-300+300*leg);
 * if((leg1Added == 1) && (leg2Added == 1)) {
 * document.getElementById("selectBooking").style.display = "block";
 * document.getElementById("priceBox").style.display = "none"; } else {
 * document.getElementById("selectBooking").style.display = "none";
 * document.getElementById("priceBox").style.display = "block"; } if((leg1Added ==
 * 1) || (leg2Added == 1)) {
 * document.getElementById("shopping_cart_panel").style.display = "block"; }
 * else { document.getElementById('shopping_cart_panel').style.display = "none"; } }
 */

function addInfant(s) {
	document.getElementById("infant" + s + "_" + infant_Count).innerHTML = '<label>Infant '
			+ infant_Count
			+ '</label><select class="w75"><option value="Age">Age</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select>';
				...
				function() {
					...
					$('#childCar1')
							.on(
									'change',
									function(e) {
										var countcarchild = $('#childCar1')
												.val();
										$('#carchildage1').html('');
										var string = '';
										for (i = 1; i <= countcarchild; i++) {
											string += '<li class="fL pL0 mT10 clearfix"> <label >Child '
													+ i
													+ '</label> <select class="mB10 " id="child_age1" ><option value="null">Age</option><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="10">10</option><option value="11">11</option><option value="12">12</option></select> </li>';
										}
...
//add by Felix for searchPage validiton -- start

function setupInputsBlurEventForSearchPage(liElementId,numOfTrips){
	var inputSelectorStr;
	if(numOfTrips){
		for(var i = 0 ; i < numOfTrips + 1 ; i++){
			addInputBlurEventForSearchPage(liElementId,i);
		}
	}else{
		addInputBlurEventForSearchPage(liElementId);
	}
}

function addInputBlurEventForSearchPage(liElementId,tripIndex){
	var inputSelectorStr;
	if(tripIndex != null){
		inputSelectorStr = 'li#' + liElementId + tripIndex + '>div>input';
	}else{
		inputSelectorStr = 'li#' + liElementId + '>div>input';
	}
	$(inputSelectorStr).attr('onBlur','validateInputEmptyValue(this)');
}

function validateInputEmptyValue(inputElement,isSubmit){
	var isEmpty = false;
	var inputElementVisible = $(inputElement).filter(":visible").last();
	if(inputElementVisible.val() == ''){
		inputElementVisible.attr('placeholder','should not be empty');
		isEmpty = true;
		if(isSubmit){
			inputElementVisible.addClass("err");
		}
	}else{
		inputElementVisible.removeClass("err");
		//console.log('inputElement',inputElementVisible.attr('class'));
	}
	return isEmpty
}
...
//add by Felix for searchPage validiton -- end